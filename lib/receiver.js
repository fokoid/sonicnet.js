const Globals = require('./globals.js'),
      ...
      RingBuffer = require('./ring-buffer.js');
 ...
 */
SonicReceiver.prototype.start = function(successCallback, rejectedCallback) {
...
SonicReceiver.prototype.onStream_ = function(stream) {
  ...
  // Setup audio graph.
  var input = Globals.audioContext.createMediaStreamSource(stream);
  var analyser = Globals.audioContext.createAnalyser();
...
SonicReceiver.prototype.indexToFreq = function(index) {
  var nyquist = Globals.audioContext.sampleRate/2;
...
SonicReceiver.prototype.analysePeaks = function() {
  ...
  if (this.state == State.IDLE) {
    ...
    if (char == this.coder.endChar) {
      this.state = State.IDLE;
      this.buffer = Globals.restoreDuplicates(Globals.restoreCapitals(this.buffer.split(''))).toString();
...
//TODO why are we reloading the page? pretty intrusive
